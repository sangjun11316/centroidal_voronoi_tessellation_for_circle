...
// MAIN
bool knn_cuda_global(float *       d_refPitch,
                     float *       d_ref,
                     int           ref_nb,
                     float *       d_queryPitch,
                     float *       d_query,  // local
                     ...
                     int           dim,
                     int           k,
                     float *       d_knn_distPitch,    // local
                     int *         d_knn_indexPitch) { // local
    ...
    err0 = cudaMemcpy2D(d_refPitch,   ref_pitch_in_bytes,   d_ref,   ref_nb * size_of_float,   ref_nb * size_of_float,   dim, cudaMemcpyDeviceToDevice);
    err1 = cudaMemcpy2D(d_queryPitch, query_pitch_in_bytes, d_query, query_nb * size_of_float, query_nb * size_of_float, dim, cudaMemcpyDeviceToDevice);
    ...
    compute_distances<<<grid0, block0>>>(d_refPitch, ref_nb, ref_pitch, d_queryPitch, query_nb, query_pitch, dim, d_knn_distPitch);
    ...
    modified_insertion_sort<<<grid1, block1>>>(d_knn_distPitch, dist_pitch, d_knn_indexPitch, index_pitch, query_nb, ref_nb, k);
    if (cudaGetLastError() != cudaSuccess) {
        printf("ERROR: Unable to execute kernel\n");
        return false;
    }
    ...
    if (k % 16 != 0)        grid2.y += 1;
    compute_sqrt<<<grid2, block2>>>(d_knn_distPitch, query_nb, query_pitch, k);	
    if (cudaGetLastError() != cudaSuccess) {
        printf("ERROR: Unable to execute kernel\n");
        return false;
    }
    ...
    err0 = cudaMemcpy2D(d_knn_distPitch,  query_nb * size_of_float, d_knn_distPitch,  dist_pitch_in_bytes,  query_nb * size_of_float, k, cudaMemcpyDeviceToDevice);
    err1 = cudaMemcpy2D(d_knn_indexPitch, query_nb * size_of_int,   d_knn_indexPitch, index_pitch_in_bytes, query_nb * size_of_int,   k, cudaMemcpyDeviceToDevice);
    if (err0 != cudaSuccess || err1 != cudaSuccess) {
    ...
    // Memory clean-up
//    cudaFree(d_refPitch);
//    cudaFree(d_queryPitch);
//    cudaFree(d_knn_distPitch);
//    cudaFree(d_knn_indexPitch); 
